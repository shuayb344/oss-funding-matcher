...
import { useAdminStore, FunderSuggestion } from "@/lib/useAdminStore";
...
export default function AdminFundersPage() {
    ...
    return (
      <div className="flex flex-1 flex-col items-center justify-center py-20 px-6 text-center">
        ...
        <p className="mt-2 text-xs font-sans text-zinc-400 max-w-md leading-relaxed">
          You do not have administrator permissions to access this control panel. Configure your GitHub ID or username in{" "}
          <code className="font-mono text-emerald-400 bg-black/60 px-1 py-0.5 border border-white/10">
            ADMIN_GITHUB_IDS
          </code>{" "}
          inside <code className="font-mono text-emerald-400 bg-black/60 px-1 py-0.5 border border-white/10">.env.local</code>.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 sm:px-6 py-12 bg-transparent min-h-[calc(100vh-3.5rem)]">
      {/* Header */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8 border-b border-slate-200 dark:border-white/10 pb-6">
        <div>
          <div className="flex items-center gap-2 font-mono text-xs uppercase tracking-widest text-emerald-600 dark:text-emerald-400 mb-1">
            ...
            // Admin Panel / Funder Registry & Moderation
          </div>
          ...
          <p className="mt-1 font-mono text-xs text-slate-500 dark:text-zinc-400">
            {funders.length} ACTIVE FUNDERS · {pendingSuggestionsCount} PENDING SUGGESTIONS
          </p>
        </div>
        ...
        {activeTab === "funders" && (
          <button
            ...
            className="inline-flex items-center gap-2 rounded-none bg-emerald-500 border border-emerald-400 px-4 py-2 font-mono text-xs font-semibold text-black hover:bg-emerald-400 hover:shadow-[0_0_12px_rgba(34,197,94,0.3)] transition-all shrink-0"
          >
      ...
      {/* Tabs */}
      <div className="flex flex-wrap border-b border-slate-200 dark:border-white/10 mb-6 font-mono text-xs">
        <button
          onClick={() => setActiveTab("funders")}
          className={`px-3 sm:px-4 py-2.5 uppercase font-semibold border-b-2 transition-all ${
            activeTab === "funders"
        ...
        >
          Active Funders ({funders.length})
        </button>
        <button
          onClick={() => setActiveTab("suggestions")}
          className={`px-3 sm:px-4 py-2.5 uppercase font-semibold border-b-2 transition-all flex items-center gap-2 ${
        ...
        >
          ...
          {pendingSuggestionsCount > 0 && (
            <span className="inline-flex items-center justify-center rounded-none bg-amber-500 text-black px-1.5 py-0.5 text-[10px] font-bold">
            ...
            <div className="space-y-4 font-sans text-xs">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full rounded-none border border-slate-300 dark:border-white/10 bg-slate-50 dark:bg-black/60 px-3 py-2 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-emerald-500 font-mono uppercase"
                  >
                    {TYPES.map((t) => (
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={addTag}
                    className="rounded-none border border-slate-300 dark:border-white/10 bg-slate-100 dark:bg-white/[0.04] px-4 py-2 font-mono text-xs text-slate-700 dark:text-zinc-300 hover:border-slate-400 dark:hover:border-white/30 hover:text-slate-900 dark:hover:text-white"
                  >
                    ADD
                  </button>
                </div>
                <div className="flex flex-wrap gap-1.5 mt-2.5">
                  {formData.focus_tags.map((tag) => (
                    ...
                    >
                      #{tag}
                      <button onClick={() => removeTag(tag)} className="text-slate-400 dark:text-zinc-500 hover:text-red-400">
                        <X className="h-3 w-3" />
                      </button>
                    </span>
                  ))}
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 sm:gap-4">
                <div className="min-w-0 flex-1">
                  <h3 className="text-sm font-semibold text-slate-900 dark:text-white font-sans leading-snug">
                    {funder.name}
                  </h3>
                  <p className="mt-1 font-mono text-xs text-emerald-600 dark:text-emerald-400 font-semibold break-words">
                    {funder.amount_range} ·{" "}
                    <span className="uppercase text-slate-500 dark:text-zinc-400">
                  ...
                  {funder.focus_tags?.length > 0 && (
                    <div className="mt-3 flex flex-wrap gap-1.5 font-mono text-[10px]">
                      {funder.focus_tags.map((tag) => (
                        <span
                          key={tag}
                          className="inline-flex items-center rounded-none border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-[#111116] px-2 py-0.5 text-slate-500 dark:text-zinc-400 uppercase"
                        >
                          #{tag}
                        </span>
                      ))}
                ...
                <div className="flex items-center gap-2 pt-2 sm:pt-0 shrink-0 font-mono text-xs border-t sm:border-t-0 border-slate-100 dark:border-white/[0.05]">
                  <button
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-none border border-red-500/30 bg-red-500/10 px-3 py-1.5 text-red-600 dark:text-red-400 hover:border-red-500/60 hover:bg-red-500/20 transition-all"
                  >
      ...
      {activeTab === "suggestions" && (
        <div className="space-y-6">
          {/* Moderation Toast Alert */}
          {lastModerationResult && (
            <div
              className={`p-4 border font-mono text-xs flex items-start gap-3 ${
                lastModerationResult.success
                  ? lastModerationResult.copiedToFunders !== false
            ...
            >
              ...
              <div className="flex-1 font-sans text-xs">
                <span className="font-mono uppercase font-bold tracking-wider block mb-0.5">
                  [ MODERATION RESULT ]
                </span>
                {lastModerationResult.message}
              </div>
            </div>
          )}
            ...
            <div className="flex items-center gap-1.5">
              {(["pending", "approved", "rejected", "all"] as const).map((st) => (
                <button
                  ...
                  className={`px-3 py-1 uppercase rounded-none border transition-all ${
                    suggestionFilter === st
                      ? "border-emerald-500 bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 font-bold"
                      : "border-slate-200 dark:border-white/10 text-slate-500 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-white"
                  }`}
                >
                  {st} (
                  {st === "all"
          ...
          {/* Suggestions List */}
          {loadingSuggestions ? (
            <div className="py-12 text-center font-mono text-xs text-slate-400 dark:text-zinc-500">
            ...
            <div className="rounded-none border border-slate-200 dark:border-white/10 bg-white dark:bg-[#0e0e12] p-12 text-center font-mono text-xs text-slate-400 dark:text-zinc-500 uppercase shadow-sm dark:shadow-none">
              No suggestions found under "{suggestionFilter}" status.
            </div>
          ) : (
            <div className="space-y-4">
              {filteredSuggestions.map((suggestion) => {
                ...
                return (
                  <div
                    key={suggestion.id}
                    className="rounded-none border border-slate-200 dark:border-white/10 bg-white dark:bg-[#0e0e12] backdrop-blur-xl p-6 transition-all shadow-sm dark:shadow-none space-y-4"
                  >
                    {/* Header */}
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 sm:gap-4">
                      <div>
                        <div className="flex flex-wrap items-center gap-2 mb-1">
                          <h3 className="text-base font-semibold text-slate-900 dark:text-white font-sans leading-snug">
                            {suggestion.name}
                          </h3>
                          <span
                            className={`px-2 py-0.5 font-mono text-[10px] uppercase font-bold border ${
                        ...
                        <a
                          href={suggestion.application_url}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-1 font-mono text-xs text-emerald-600 dark:text-emerald-400 hover:underline break-all"
                        >
                          {suggestion.application_url} <ExternalLink className="h-3 w-3 shrink-0" />
                        </a>
                      </div>
                      ...
                      {suggestion.status === "pending" && (
                        <div className="flex items-center gap-2 pt-2 sm:pt-0 shrink-0 font-mono text-xs border-t sm:border-t-0 border-slate-100 dark:border-white/[0.05]">
                          <button
                            ...
                            className="inline-flex items-center gap-1.5 rounded-none bg-emerald-500 border border-emerald-400 px-3.5 py-1.5 font-semibold text-black hover:bg-emerald-400 transition-all disabled:opacity-50"
                          >
                            {isProcessing ? (
                              <>
                                <Loader2 className="h-3.5 w-3.5 animate-spin" /> CHECKING URL…
                              </>
                            ) : (
                              <>
                          ...
                          <button
                            ...
                            disabled={isProcessing}
                            className="inline-flex items-center gap-1.5 rounded-none border border-red-500/40 bg-red-500/10 px-3 py-1.5 font-semibold text-red-600 dark:text-red-400 hover:bg-red-500/20 transition-all disabled:opacity-50"
                          >
                    ...
                    <div className="flex flex-wrap items-center gap-4 text-xs font-mono">
                      {suggestion.focus_tags?.length > 0 && (
                        <div className="flex items-center gap-1.5 text-[11px]">
                          <Tag className="h-3 w-3 text-slate-400 dark:text-zinc-500" />
                          {suggestion.focus_tags.map((tag) => (
                            <span
                              key={tag}
                              className="border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-[#111116] px-1.5 py-0.5 text-slate-600 dark:text-zinc-400 uppercase"
                            >
                              #{tag}
                            </span>
                          ))}
                      ...
                      {suggestion.notes && (
                        <div className="flex items-center gap-1.5 text-[11px] text-amber-700 dark:text-amber-400 bg-amber-500/10 border border-amber-500/20 px-2 py-0.5">
                          <FileText className="h-3 w-3" />
                          <span>Notes: {suggestion.notes}</span>
                        </div>
                      )}
                    </div>

                    {/* Footer Metadata */}
                    <div className="pt-3 border-t border-slate-200 dark:border-white/10 flex items-center justify-between font-mono text-[11px] text-slate-400 dark:text-zinc-500">
                      <div className="flex items-center gap-1.5">
                        <User className="h-3 w-3" />
                        <span>
                          SUBMITTED BY:{" "}
                          <strong className="text-slate-700 dark:text-zinc-300">
                            {suggestion.users?.username || "Community Member"}
                          </strong>
                        </span>
                      </div>
                      <div className="flex items-center gap-1.5">
